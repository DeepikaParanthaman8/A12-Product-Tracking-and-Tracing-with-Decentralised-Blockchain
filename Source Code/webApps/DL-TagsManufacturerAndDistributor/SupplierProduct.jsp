...
<%@page import="com.Modul.SupplierRegPojo"%>
<%@page import="com.Modul.AddProduct"%>
<!DOCTYPE HTML>
<html>
<head>
...
<div class="women_main">
   ...
   <div class="w_content">
		...
		<%
		  ...
		  String supid=(String)session.getAttribute("uname");
		  String manifacturename=(String)session.getAttribute("name");
		  Dao dao=new Dao(); 
		  List<AddProduct> list=dao.getProductDetails(supid);
		  for(AddProduct pojo:list){
			  if(Integer.parseInt(pojo.getQuanity())!=0){
		  ...
		  <div class="grid1_of_4">
				...
				<div class="content_box"><a href="#">
			   	   	 <img src="http://localhost:8888/<%=pojo.getImage().replace("webapps","")%>" class="img-responsive" alt="" width="40%" height="40%">
				   	  </a>
				    
				    <p>Product_Name:<%=pojo.getProductname()%></p>
				    <p>Description:<%=pojo.getDescription()%></p>
				     <p>Product_Id:<%=pojo.getProid()%></p>
				      <p>Price:<%=pojo.getPrice()%></p>
				      <p>Quantity:<%=pojo.getQuanity()%></p>
				      
				      <form action="ManufactureRequest" method="post"> 
					 
					 
					 <input type="hidden" name="name" value="<%=manifacturename%>">
					 <input type="hidden" name="supid" value="<%=pojo.getSuplierid()%>">
					 <input type="hidden" name="proid" value="<%=pojo.getProid()%>">
					 <input type="hidden" name="pname" value="<%=pojo.getProductname()%>">
					 <input type="number" name="quanity" onkeyup="validate(<%=pojo.getQuanity()%>,this.value,document.getElementById('<%=pojo.getProid()%>'))" min="1" max="<%=pojo.getQuanity()%>"/>
					 <input type="submit" value="Click" id="<%=pojo.getProid()%>" style="background-color: sienna;color: white;">
